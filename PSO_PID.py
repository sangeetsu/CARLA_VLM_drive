...
# Return:
# fitArr - 1/MSE of trajectory error * 0.5 + 1/MSE of velocity error * 0.5 from all solutions
def CalcFit(solution):
    print("Current Solution: ",solution)
    fitArr = []
    saveProgress(solution)
    for sol in solution:
        error = run_simulator(sol)
        # print("ERROR: ",error)
        ...
        finalFit = ((0.5*trajE)+(0.5*velE))
        fitArr.append(finalFit)
    return fitArr
...
def load_gains(participant_id):
    filename = 'JSONBASE/'+participant_id + '_PSOgains.json'
    try:
...
# Return:
#   No return
def update_pids_json(participant_id, solution, Fitness):
    filename = 'JSONBASE/' + participant_id + '_PSOgains.json'
...
# This saves to a global variable the current solution, so we can write the information to a file, and then
# recall that information to bypass the Page limit memory leak in CARLA
# Params:
#   solution - the set of all solutions
# Return:
#   No Return Value
def saveProgress(solution):
...
if __name__ == "__main__":
    ...
    argparser = argparse.ArgumentParser(description='PSO File Processor')
    ...
    argparser.add_argument(
            '-n', '--New',
            action='store_true',
            help='pass a true if new PSO needs to be started')
    ...
    try:
        if args.New:
            ...
            update_pids_json(args.ID, solution, fitness) # Update the JSON file with the new gains
            np.savetxt(writepath, current_solution, fmt='%f')
        else:
            throttle_brake_gains, steering_gains, safety_buffer, speed_adhere = load_gains(args.ID)
            readpath = "participantPSO/"+args.ID+"PSOSAVE.txt"
            nested_lists = np.loadtxt(readpath)
            print(nested_lists)
            initPop = nested_lists
            initPop[0] = np.array([throttle_brake_gains.kp, throttle_brake_gains.ki, throttle_brake_gains.kd,
                       steering_gains.kp, steering_gains.ki, steering_gains.kd, safety_buffer, speed_adhere])
            print(initPop)
            constraints = (np.array([0,0,0,0,0,0,0,-15]),np.array([5,5,5,5,5,5,5,15]))
