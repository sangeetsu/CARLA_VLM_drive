...
def is_within_school_zone(x, y, start, end):
...
def calculate_r_squared_nearest(participant_id, simulation_csv_path, human_trajectory_csv_path, viz_dir):
    ...
    #Define School Zone area
    school_zone = landmarks_df[landmarks_df['property'] == 'SZ']
    # school_zone_start = (0.01*school_zone['x'].values[0], 0.01*school_zone['y'].values[0])
    # school_zone_end = (0.01*school_zone['x'].values[1], 0.01*school_zone['y'].values[1])
    school_zone_start = (0.01 * school_zone['x'].values[0], -60)
    school_zone_end = (0.01 * school_zone['x'].values[1], -40)
    ...
    # Highlight the school zone area
    for i, in_zone in enumerate(school_zone_indices):
        if in_zone:
